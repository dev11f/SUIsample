        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 15) {
                ...
                VStack(spacing: 15) {
                    ...
                    Text(expenseViewModel.convertExpensesToCurreny(expenses: expenseViewModel.expenses, type: expenseViewModel.tabName))
                        .font(.title.bold())
                        .opacity(0.9)
                ...
                ForEach(expenseViewModel.expenses.filter({ $0.type == expenseViewModel.tabName
                })) { expense in
        ...
        HStack(spacing: 0) {
            ForEach([ExpenseType.income, ExpenseType.expense], id: \.rawValue) { tab in
                Text(tab.rawValue.capitalized)
                    ...
                    .frame(maxWidth: .infinity)
                    .background {
                        // MARK: With Mached Geometry Effect
