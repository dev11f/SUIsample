...
class ExpenseViewModel: ObservableObject {
    ...
    func currentMonthDateString() -> String {
        currentMonthStartDate.formatted(date: .abbreviated, time: .omitted) + " - " + Date().formatted(date: .abbreviated, time: .omitted)
    }
    
    func convertExpensesToCurreny(expenses: [Expense], type: ExpenseType = .all) -> String {
        var value: Double = 0
